...
const scoreColor = (s) => s >= 8 ? "text-emerald-400" : s >= 6 ? "text-amber-400" : "text-rose-400";
const scoreBg    = (s) => s >= 8 ? "bg-emerald-500/15 border-emerald-500/30" : s >= 6 ? "bg-amber-500/15 border-amber-500/30" : "bg-rose-500/15 border-rose-500/30";
...
const parseMCQ = (text) => {
  ...
  const options  = lines.filter(l => /^[A-D]\./.test(l)).map(l => ({ key:l[0], text:l.slice(2).trim() }));
  const correct  = lines.find(l => l.startsWith("Correct Answer:"))?.split(":")[1]?.trim();
...
export default function Interview() {
  ...
  const api = async (body) => {
    const res  = await fetch("/api/interview", { method:"POST", headers:{"Content-Type":"application/json"}, body:JSON.stringify({ ...body, resumeData:parsed }) });
  ...
  const startSession = async () => {
    setPhase("loading"); setHistory([]); setError(""); resetTimer();
    try {
      const { question, questionNumber } = await api({ action:"question", type:TYPES[selected].name, history:[] });
      setCurrentQ(question); setQuestionNum(questionNumber); setAnswer(""); setPhase("question");
  ...
  const submitAnswer = async () => {
    ...
    clearInterval(timerRef.current); setTimerActive(false); setPhase("loading"); setError("");
    try {
      const { feedback: fb } = await api({ action:"feedback", type:TYPES[selected].name, currentQuestion:currentQ, userAnswer:answer.trim() || "(No answer given — time ran out)" });
      setFeedback(fb); setPhase("feedback");
    } catch(e) { setError(e.message); setPhase("question"); }
  };
  ...
  const nextQuestion = async () => {
    ...
    if (newHistory.length >= TOTAL_QUESTIONS) {
      setPhase("loading");
      try {
        const { report: r } = await api({ action:"report", type:TYPES[selected].name, history:newHistory });
        setReport(r); setPhase("report");
      } catch(e) { setError(e.message); setPhase("feedback"); }
      return;
    }
    setPhase("loading"); resetTimer(); setFeedback(null);
    try {
      const { question, questionNumber } = await api({ action:"question", type:TYPES[selected].name, history:newHistory });
      setCurrentQ(question); setQuestionNum(questionNumber); setAnswer(""); setPhase("question");
      setTimeout(() => { setTimerActive(true); textareaRef.current?.focus(); }, 300);
    } catch(e) { setError(e.message); setPhase("feedback"); }
  };
  ...
  const endSession = () => {
    clearInterval(timerRef.current);
    setPhase("setup"); setHistory([]); setCurrentQ(""); setAnswer("");
    setFeedback(null); setReport(null); setError(""); resetTimer();
  };

  const avgScore = history.length > 0
    ? Math.round(history.reduce((s,h) => s + (h.feedback?.score || 0), 0) / history.length) : 0;

  // ── Progress dots ──
  const ProgressDots = ({ current }) => (
    <div className="flex gap-1">
      {Array.from({ length:TOTAL_QUESTIONS }).map((_,i) => (
        <div key={i} className={`h-1 rounded-full transition-all ${i < current-1 ? "w-5 bg-amber-400" : i===current-1 ? "w-5 bg-amber-400/60" : "w-2.5 bg-white/10"}`} />
      ))}
    </div>
  );
  ...
  if (phase === "report" && report) return (
    <div className="min-h-screen relative overflow-hidden py-8 px-4">
      <Orbs />
      <div className="relative z-10 w-full max-w-2xl mx-auto border border-white/10 rounded-3xl p-5 sm:p-8 shadow-2xl backdrop-blur-[1px]">
        <div className="text-center mb-6">
          <p className="text-[10px] uppercase tracking-widest text-white/30 mb-2">Session Complete</p>
          <h1 className="text-2xl sm:text-3xl font-bold text-white mb-1">Interview <span className="bg-gradient-to-r from-amber-400 to-orange-500 bg-clip-text text-transparent italic">Report</span></h1>
          <p className="text-white/30 text-sm">{TYPES[selected].name} · {TOTAL_QUESTIONS} questions</p>
        </div>

        <div className={`rounded-2xl border p-5 mb-5 text-center ${scoreBg(report.overallScore)}`}>
          <p className={`text-5xl sm:text-6xl font-black mb-1 ${scoreColor(report.overallScore)}`}>{report.overallScore}<span className="text-xl sm:text-2xl">/10</span></p>
          <p className="text-white font-semibold text-base sm:text-lg">{report.overallLabel}</p>
    ...
    <div className="min-h-screen relative overflow-hidden py-8 px-4">
      ...
      <div className="relative z-10 w-full max-w-2xl mx-auto border border-white/10 rounded-3xl p-5 sm:p-8 shadow-2xl backdrop-blur-[1px]">
        <div className="flex items-center justify-between mb-6">
          <p className="text-[10px] text-white/25 uppercase tracking-widest">Q{questionNum}/{TOTAL_QUESTIONS}</p>
        ...
        <div className="rounded-xl bg-white/[0.03] border border-white/8 p-4 mb-3">
        ...
        <div className="rounded-xl bg-white/[0.02] border border-white/6 p-4 mb-3">
        ...
        <div className="flex gap-2 sm:gap-3">
          <button onClick={endSession} className="flex-1 py-3 rounded-xl font-medium text-sm text-white/40 border border-white/10 hover:border-white/20 transition-all">End</button>
          <button onClick={nextQuestion} className="flex-[2] py-3 rounded-xl font-medium text-sm text-gray-900 bg-gradient-to-r from-amber-400 to-orange-500 shadow-lg shadow-amber-500/20 hover:-translate-y-0.5 transition-all">
            {history.length + 1 >= TOTAL_QUESTIONS ? "View Report →" : `Next (${history.length+1}/${TOTAL_QUESTIONS}) →`}
          </button>
        </div>
      </div>
    </div>
      ...
      <div className="min-h-screen relative overflow-hidden py-8 px-4">
        ...
        <div className="relative z-10 w-full max-w-2xl mx-auto border border-white/10 rounded-3xl p-5 sm:p-8 shadow-2xl backdrop-blur-[1px]">
          ...
          <div className="flex items-center justify-between mb-5">
            <ProgressDots current={questionNum} />
            <div className={`flex items-center gap-2 text-sm font-mono font-semibold ${timeLeft<=30 ? "text-rose-400" : "text-white/40"}`}>
              <div className="relative w-6 h-6">
                <svg className="w-6 h-6 -rotate-90" viewBox="0 0 24 24">
                  <circle cx="12" cy="12" r="10" fill="none" stroke="rgba(255,255,255,0.08)" strokeWidth="2" />
                  <circle cx="12" cy="12" r="10" fill="none" stroke={timeLeft<=30 ? "#f87171" : "#fbbf24"} strokeWidth="2"
                    strokeDasharray={`${(timerPct/100)*62.8} 62.8`} strokeLinecap="round" />
                </svg>
              </div>
              {fmtTime(timeLeft)}
            </div>
          ...
          <div className="flex items-center gap-2 mb-4 flex-wrap">
            ...
            <span className="text-[11px] text-white/25 uppercase tracking-widest">{TYPES[selected].name} · Q{questionNum}</span>
            {parsed?.targetRole && (
              <span className="ml-auto text-[10px] px-2 py-0.5 rounded-full bg-amber-500/10 border border-amber-500/20 text-amber-400/70">{parsed.targetRole}</span>
            )}
          </div>

          <div className="rounded-2xl border border-amber-500/20 bg-amber-500/5 p-4 mb-5">
            <p className="text-white text-sm sm:text-base leading-relaxed font-medium">{mcq?.question || currentQ}</p>
          </div>

          {mcq?.options?.length > 0 ? (
            <div className="mb-5">
              <label className="text-[10px] text-white/25 uppercase tracking-widest block mb-3">Choose your answer</label>
              <div className="space-y-2.5">
                {mcq.options.map(opt => (
                  <button key={opt.key} onClick={() => setAnswer(opt.key)}
                    className={`w-full text-left px-4 py-3 rounded-xl border transition-all text-sm ${answer===opt.key ? "border-amber-400 bg-amber-500/10 text-white" : "border-white/10 bg-white/[0.03] text-white/70 hover:border-white/20"}`}>
            ...
            <div className="mb-5">
              <label className="text-[10px] text-white/25 uppercase tracking-widest block mb-2">Your Answer</label>
              <textarea ref={textareaRef} value={answer} onChange={e => setAnswer(e.target.value)}
                ...
                rows={5}
                className="w-full bg-white/[0.03] border border-white/10 rounded-xl px-4 py-3 text-white text-sm placeholder:text-white/20 outline-none focus:border-amber-400/30 resize-none leading-relaxed transition-colors" />
              <div className="flex justify-between mt-1.5">
                <p className="text-[10px] text-white/15">{answer.length} characters</p>
                {TYPES[selected].name === "Behavioral" && <p className="text-[10px] text-white/20">Tip: Use the STAR method</p>}
              </div>
            </div>
          )}
          ...
          {history.length > 0 && (
            <div className="flex items-center gap-2 mb-4">
              ...
              <span className={`text-[11px] font-bold ${scoreColor(avgScore)}`}>{avgScore}/10</span>
              <div className="flex gap-1 ml-1">
                {history.map((h,i) => (
                  <div key={i} className={`w-1.5 h-1.5 rounded-full ${h.feedback?.score>=8?"bg-emerald-400":h.feedback?.score>=6?"bg-amber-400":"bg-rose-400"}`} />
                ))}
              </div>
            </div>
          )}
          ...
          <div className="flex gap-2 sm:gap-3">
            <button onClick={endSession} className="px-3 sm:px-4 py-3 rounded-xl font-medium text-sm text-white/35 border border-white/10 hover:border-white/20 transition-all flex-shrink-0">End</button>
            <button onClick={submitAnswer} disabled={!answer.trim()}
              className="flex-1 py-3 rounded-xl font-medium text-sm text-gray-900 bg-gradient-to-r from-amber-400 to-orange-500 shadow-lg shadow-amber-500/20 hover:-translate-y-0.5 active:translate-y-0 transition-all disabled:opacity-40 disabled:cursor-not-allowed">
              Submit Answer →
            </button>
          </div>
        </div>
  ...
  return (
    <div className="min-h-screen relative overflow-hidden px-4 py-8">
      <Orbs />
      <div className="relative z-10 w-full max-w-2xl mx-auto border border-white/10 backdrop-blur-[1px] rounded-3xl p-6 sm:p-10 shadow-2xl">

        <h1 className="text-3xl sm:text-4xl font-bold text-white leading-tight tracking-tight">
          Master Your{" "}
          <span className="italic bg-gradient-to-r from-amber-400 to-orange-500 bg-clip-text text-transparent">Interview</span>
        </h1>
        <p className="mt-3 text-sm text-white/70 font-light">Practice with our AI interviewer and get instant, detailed feedback.</p>
        ...
        {parsed?.fullName ? (
          ...
          <div className="mt-4 inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-white/5 border border-white/10">
            ...
            <p className="text-[11px] text-white/30">Generic questions · <a className="text-amber-400/70 underline underline-offset-2">Upload resume</a> for personalised session</p>
          </div>
        )}
        ...
        <div className="grid grid-cols-1 gap-3 sm:grid-cols-3">
          {TYPES.map((t,i) => (
            <button key={i} onClick={() => setSelected(i)}
              className={`relative text-left rounded-2xl p-4 border transition-all duration-200 ${selected===i ? "border-amber-400 bg-amber-400/10 shadow-lg shadow-amber-500/10" : "border-white/10 bg-white/5 hover:border-amber-400/30 hover:bg-amber-400/5"}`}>
        ...
        <div className="mt-5 grid grid-cols-3 gap-2">
          {[{ label:"Questions", value:`${TOTAL_QUESTIONS} total` },{ label:"Time/Q", value:"2 minutes" },{ label:"Feedback", value:"Instant AI" }].map((s,i) => (
        ...
        <p className="text-center text-xs text-white/25 mt-4">{TOTAL_QUESTIONS} questions · ~{TOTAL_QUESTIONS*2} min · Powered by GPT-4o mini</p>
      </div>
    </div>
  );
}